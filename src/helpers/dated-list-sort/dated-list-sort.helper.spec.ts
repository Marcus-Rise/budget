import {
  DATED_LIST_SORT_KEY,
  datedListSortHelper,
  datedListSortKeyToDateHelper,
} from "./dated-list-sort.helper";
import format from "date-fns/format";

describe("datedListSortHelper", () => {
  it("should return grouped object", () => {
    ...
    const dateGroups = datedListSortHelper(list);
...
describe("datedListSortKeyToDateHelper", () => {
  it("should restore date", () => {
    const key = format(new Date(2022, 1, 12), DATED_LIST_SORT_KEY);

    expect(format(datedListSortKeyToDateHelper(key), DATED_LIST_SORT_KEY)).toEqual("2022-02-12");
  });
});
